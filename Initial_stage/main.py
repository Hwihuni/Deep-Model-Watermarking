...
parser.add_argument('--trainpics', default='../HR/',
                    help='folder to output training images')
parser.add_argument('--validationpics', default='../HR/',
                    help='folder to output validation images')
parser.add_argument('--testPics', default='../HR/',
                    help='folder to output test images')
parser.add_argument('--runfolder', default='../HR/',
                    help='folder to output test images')
parser.add_argument('--outckpts', default='../HR/',
                    help='folder to output checkpoints')
parser.add_argument('--outlogs', default='../HR/',
                    help='folder to output images')
parser.add_argument('--outcodes', default='../HR/',
...
#datasets to train
parser.add_argument('--datasets', type=str, default='../data_wm/LR_1e-05_epoch353/',
                    help='denoise/derain')
...
parser.add_argument('--num_downs', type=int, default= 0 , help='nums of  Unet downsample')
parser.add_argument('--clip', action='store_true', help='clip container_img')
parser.add_argument('--gpu_ind', type=str, default='0,1',help='gpu')
...
def main():
    ...
    traindir = os.path.join(DATA_DIR, 'train.mat')
    valdir = os.path.join(DATA_DIR, 'val.mat')

    
    train_dataset = BasicDataset(traindir,4,0.08)
    val_dataset = BasicDataset(valdir,4,0.08)
...
def train(train_loader,  epoch, Hnet, Rnet, Dnet):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        cover_img_A = cover_img[ :, :, 0:320, 0:320]
        cover_img_B = cover_img[ :, :, 0:320, 320:640]
        ...
        if opt.cuda:
            #cover_img = cover_img.type(torch.FloatTensor).cuda()
            cover_img_A = cover_img_A.type(torch.FloatTensor).cuda()
